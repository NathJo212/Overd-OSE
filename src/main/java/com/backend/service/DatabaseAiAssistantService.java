...
@Service
public class DatabaseAiAssistantService {
    ...
    private static final Pattern GENERIC_OFFER_QUERY = Pattern.compile("(?i)offre(?: de stage)?");

    private static final int MAX_LIST = 5;

    public DatabaseAiAssistantService(OffreRepository offreRepository,
    ...
    private String postProcess(String raw, boolean fr, String ctx) {
        ...
        // Supprimer les hallucinations de refus d'accès
        raw = raw
                .replaceAll("(?i)je\\s+ne\\s+peux\\s+pas\\s+acc[eè]der.*", "")
                .replaceAll("(?i)je\\s+n'?ai\\s+pas\\s+acc[eè]s.*", "")
                .replaceAll("(?i)i\\s+(?:do\\s+not|don't)\\s+have\\s+access.*", "")
                .trim();
