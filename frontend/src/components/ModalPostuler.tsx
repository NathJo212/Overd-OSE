...
const ModalPostuler = ({ offreId, offreTitre, onClose, onSuccess, onPostuler }: ModalPostulerProps) => {
    ...
    return (
        ...
        >
            <div className="bg-white dark:bg-slate-800 w-full max-w-lg rounded-2xl shadow-2xl p-6 animate-fade-in border border-gray-200 dark:border-slate-700">
                <div className="flex items-center justify-between mb-4">
                    ...
                    <button
                        onClick={onClose}
                        className="cursor-pointer text-gray-400 dark:text-slate-400 hover:text-gray-600 dark:hover:text-slate-200 transition-colors"
                        disabled={loading}
                    >
                        <X className="w-5 h-5" />
                    </button>
                ...
                <div className="mb-4">
                    ...
                    <p className="font-semibold text-gray-900 dark:text-slate-100">{offreTitre}</p>
                </div>

                {error && (
                    <div className="mb-4 p-3 bg-red-50 dark:bg-red-900/20 rounded-lg border border-red-200 dark:border-red-900/40 flex items-start gap-2">
                ...
                <div className="mb-6">
                    ...
                    <label
                        ...
                        className="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-slate-300 dark:border-slate-600 rounded-xl cursor-pointer bg-slate-50 dark:bg-slate-700/40 hover:bg-slate-100 dark:hover:bg-slate-600 transition-colors"
                    >
                        <div className="flex flex-col items-center justify-center pt-5 pb-6">
                            {lettreMotivation ? (
                                <>
                                    <CheckCircle className="w-8 h-8 text-green-500 mb-2" />
                                    <p className="text-sm text-slate-600 dark:text-slate-200 font-semibold">
                                ...
                                <>
                                    <Upload className="w-8 h-8 text-slate-400 dark:text-slate-300 mb-2" />
                    ...
                    {lettreMotivation && (
                        <button
                            ...
                            disabled={loading}
                            className="cursor-pointer mt-2 text-sm text-red-600 dark:text-red-400 hover:text-red-700 dark:hover:text-red-300 transition-colors"
                        >
                ...
                <div className="bg-blue-50 dark:bg-blue-900/30 border border-blue-200 dark:border-blue-900/40 rounded-lg p-3 mb-6">
                ...
                <div className="flex gap-3">
                    <button
                        ...
                        disabled={loading}
                        className="cursor-pointer flex-1 bg-blue-600 hover:bg-blue-700 disabled:bg-slate-300 dark:disabled:bg-slate-600 text-white font-medium py-3 rounded-xl transition-all duration-200 shadow-md hover:shadow-lg hover:shadow-blue-400 disabled:shadow-none"
                    >
                        {loading ? (
                    ...
                    <button
                        onClick={onClose}
                        disabled={loading}
                        className="cursor-pointer flex-1 bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:hover:bg-slate-600 disabled:bg-slate-200 dark:disabled:bg-slate-600 text-gray-800 dark:text-slate-200 font-medium py-3 rounded-xl transition-all duration-200"
                    >
